  ...
  mappingValue $
    byKeyMapping (CaseSensitive True) $
      Project
        <$> atByKey "org" name
        <*> atByKey "name" name
        <*> (atByKey "migrations-dir" path <|> pure Defaults.migrationsDir)
        <*> (atByKey "queries-dir" path <|> pure Defaults.queriesDir)
        <*> (atByKey "output-dir" path <|> pure Defaults.outputDir)
